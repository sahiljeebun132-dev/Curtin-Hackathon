...
import { cacheSet } from "../secure.js";
...
export default function AssessmentFlow({ seed, onResult, savedResult, onSaveResult }) {
  ...
  async function handleQuestionnaire(formData) {
    ...
    if (self) {
      // Persist the WHOLE result (not just the level) so the patient can always
      // reopen their latest check-in from the Results tab - even after navigating
      // away or refreshing the page. Encrypted at rest via secure.js.
      cacheSet("last_level", withSummaries.risk_profile.overall_risk_level);
      cacheSet("last_result", withSummaries);
      cacheSet("last_result_at", Date.now());
    }
    setTimeout(() => {
