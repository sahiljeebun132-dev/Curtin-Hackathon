...
export default function Support() {
  ...
  useEffect(() => {
    ...
    AWARENESS_ZONES.forEach((z) => {
      L.circleMarker([z.lat, z.lng], { radius: 11, color: color(z.level), fillColor: color(z.level), fillOpacity: 0.4, weight: 2 })
        .addTo(map).bindPopup(`<b>${z.name}</b><br/>Awareness zone (${z.level})`);
    });
    SERVICE_POINTS.forEach((s) => {
      L.circleMarker([s.lat, s.lng], { radius: 9, color: "#0e9488", fillColor: "#0e9488", fillOpacity: 0.8, weight: 2 })
        .addTo(map).bindPopup(`<b>${s.name}</b><br/>Support service`);
    });
  ...
  return (
    <section className="card">
      ...
      <p className="tiny muted" style={{ marginTop: 8 }}>{t("support_map_note")}</p>
